...
class GoogleDriveService
  ...
  def ensure_session
    ...
    if e.message.downcase == 'insufficient permission'
      raise ConfigError, 'Google Drive API says we have insufficient permission to access the specified sheet ID.'
    end
    raise e
  end
  ...
  def refresh_access_token
    fail OAuth2Error, 'No refresh token available.' if refresh_token.nil?
    auth = authorization
    ...
    raise GoogleDriveService::OAuth2Error, message
  end
